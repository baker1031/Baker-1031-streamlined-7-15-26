...
2. `GET /.netlify/functions/ghl-hubspot-backfill?secret=...&mode=dry-run&limit=5` audits a small batch without CRM writes; the endpoint caps each request at five contacts to stay within the synchronous function timeout. Use the returned `nextStartAfterId` and `nextStartAfter` values on the next request to continue through the list.
